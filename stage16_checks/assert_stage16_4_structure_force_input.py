#!/usr/bin/env python3
"""Stage 16.4 structure-force-input audit helper."""
...
NUMERIC_KEYS = {"feedback_alpha", "action_reaction_error"}
...
def active_shell_has_rg_command(script: Path) -> bool:
    ...
    for raw in read_text(script).splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        # Strip simple trailing comments; command detection must not treat quoted
        # regex text such as 'rg[[:space:]]' as a real rg command.
...
def stage14_lambda_zero_gate_found(repo: Path) -> bool:
    # Only executable/source evidence is scanned. Markdown prohibition text and
    # negative-check strings must not be misclassified as a real Stage 14
    # lambda==0 hook-registration gate regression.
    ...
    return any(pattern.search(read_text(candidate)) for candidate in files)


def old_stage13_5_production_name_reappeared(repo: Path) -> bool:
    # Stage 13.5 conservation-sign audit is legitimate. Only the old production
    # force-density candidate diagnostic name is forbidden; documentation and
    # negative-audit strings in Stage 15/16 must not be treated as regressions.
    files = [
    ...
    return any("stage13_5_production_force_density_candidate" in read_text(candidate) for candidate in files if candidate.exists())
...
def main() -> int:
    ...
    parser.add_argument("--require-stage16-3", default="1")
    parser.add_argument("--accept-stage16-3-closed-evidence", default="1")
    ...
    dat = out_dir / "fibre_stage16_4_structure_force_input.dat"
    reasons_file = out_dir / "stage16_4_structure_force_input_reasons.tmp"
    data = parse_dat(dat)
    reasons: list[str] = []
    ...
    if args.build_status != "1":
        reasons.append("stage16_4_structure_force_input_target_build_status_not_pass")
    if args.run_status != "1":
        reasons.append("stage16_4_structure_force_input_target_run_status_not_pass")

    wrapper = repo / "stage16_checks" / "run_stage16_4_structure_force_input.sh"
    doc = repo / "stage16_checks" / "stage16_4_structure_force_input.md"
    helper = repo / "stage16_checks" / "assert_stage16_4_structure_force_input.py"
    force_src = repo / "src" / "fibre_stage16_structure_force_input.f90"
    check_src = repo / "src" / "fibre_stage16_structure_force_input_check.f90"
    cmake = repo / "src" / "CMakeLists.txt"
    for path in [wrapper, doc, helper, force_src, check_src]:
        if not path.exists():
            reasons.append(f"missing_required_stage16_4_file_{path}")
    if "add_executable(fibre_stage16_structure_force_input_check" not in read_text(cmake):
        reasons.append("missing_fibre_stage16_structure_force_input_check_build_registration")
    ...
    if args.require_stage16_3 == "1":
        stage16_3 = parse_dat(repo / "stage16_outputs" / "fibre_stage16_3_force_sign_audit.dat")
        stage16_3_files_ok = all(path.exists() for path in stage16_3_files)
        if stage16_3.get("final_status") != "1" and not (args.accept_stage16_3_closed_evidence == "1" and stage16_3_files_ok):
            reasons.append("missing_or_failed_stage16_3_force_sign_evidence")
    ...
    if any(script_has_rg_only_dependency(candidate) for candidate in scripts_to_check):
    ...
    for path in stage16_2_files:
        if not path.exists():
            reasons.append(f"missing_stage16_2_evidence_file_{path}")
            stage16_2_regression_status = 0
    if "add_executable(fibre_stage16_one_fibre_case_check" not in read_text(cmake):
        reasons.append("stage16_2_case_check_build_registration_missing")
        stage16_2_regression_status = 0

    stage16_3_regression_status = 1
    for path in stage16_3_files:
        if not path.exists():
            reasons.append(f"missing_stage16_3_evidence_file_{path}")
            stage16_3_regression_status = 0
    if "add_executable(fibre_stage16_force_sign_audit_check" not in read_text(cmake):
        reasons.append("stage16_3_force_sign_check_build_registration_missing")
        stage16_3_regression_status = 0
    ...
    if "stage16" in read_text(repo / "src" / "xcompact3d.f90").lower():
        ...
        boundary_status = 0
    stage16_4_src = read_text(force_src) + "\n" + read_text(check_src)
    ...
    for reason, pattern in forbidden_call_patterns.items():
        if re.search(pattern, stage16_4_src, re.I):
            reasons.append(reason)
            boundary_status = 0
    update_patterns = {
        "position_update_detected": r"\b(x_f|xf|position)\s*=\s*[^=]",
        "velocity_update_detected": r"\b(v_f|vf|velocity)\s*=\s*[^=]",
        "acceleration_update_detected": r"\b(a_f|af|acceleration)\s*=\s*[^=]",
    }
    for reason, pattern in update_patterns.items():
        if re.search(pattern, stage16_4_src, re.I):
            reasons.append(reason)
            boundary_status = 0
    ...
    if boundary_status == 0:
        for key in ["no_production_hook_status", "no_structure_advance_status", "no_position_update_status", "no_velocity_update_status",
                    "no_acceleration_update_status", "no_rhs_modification_status", "no_bending_solve_status", "no_tension_solve_status",
                    "no_wall_contact_status", "no_multifibre_status", "no_pressure_projection_modification_status",
    ...
    with dat.open("w") as handle:
        handle.write("# Stage 16.4 structure-side force-input summary\n")
        for key in REQUIRED_KEYS:
            handle.write(f"{key} {data.get(key, '0')}\n")
        for key in ["no_legacy_ibm_forcing_status", "numeric_parse_status", "numeric_bounds_status", "diagnostic_only_status"]:
    ...
    if final_ok:
        print("STAGE 16.4 STRUCTURE FORCE INPUT VERDICT: PASS")
        print("STAGE 16.4 FINAL VERDICT: PASS")
        return 0
    print("STAGE 16.4 STRUCTURE FORCE INPUT VERDICT: FAIL")
    print("STAGE 16.4 FINAL VERDICT: FAIL")
    print("Reasons:")
    for reason in reasons:
        print(f" - {reason}")
    return 1
