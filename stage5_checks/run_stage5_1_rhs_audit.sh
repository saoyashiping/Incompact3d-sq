...
cmake --build build_stage5 --target fibre_stage5_rhs_audit_check -j
exe="build_stage5/bin/fibre_stage5_rhs_audit_check"
if [[ ! -x "$exe" ]]; then
  exe="$(find build_stage5 -type f -name 'fibre_stage5_rhs_audit_check' | head -n 1)"
fi
"$exe"
cat stage5_outputs/fibre_stage5_rhs_audit_check.dat
