module fibre_stage16_lambda0_no_contamination
  ...
  public :: stage16_lambda0_no_contamination_reset
  public :: stage16_lambda0_no_contamination_load_from_environment
  public :: stage16_lambda0_no_contamination_initialize
  public :: stage16_lambda0_no_contamination_apply_one_step
  public :: stage16_lambda0_no_contamination_validate
  public :: stage16_lambda0_no_contamination_write_diagnostics
  public :: stage16_lambda0_no_contamination_get_final_status
  ...
  real(mytype) :: max_rhs_increment_value = 1.0e-14_mytype
  real(mytype) :: max_fluid_delta_value = 1.0e-14_mytype
  ...
  integer :: fluid_signature_unchanged_status = 0
...
contains
  ...
  subroutine stage16_lambda0_no_contamination_reset()
    requested_status = 1
    np_value = 1
    npts_value = 8
    ...
    max_rhs_increment_value = 1.0e-14_mytype
    max_fluid_delta_value = 1.0e-14_mytype
    ...
    fluid_signature_unchanged_status = 0
    ...
    final_status = 0
  end subroutine stage16_lambda0_no_contamination_reset

  subroutine stage16_lambda0_no_contamination_load_from_environment()
    logical :: parsed_bool
    integer :: parse_status

    call stage16_lambda0_no_contamination_reset()
    call load_bool_env('STAGE16_6_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) requested_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_6_ONE_FIBRE_FSI_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) one_fibre_fsi_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_6_STRUCTURE_UPDATE_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) structure_update_enable_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_6_TWO_WAY_RHS_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) two_way_rhs_enable_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_6_DIAGNOSTIC_ONLY', parsed_bool, parse_status)
    if (parse_status == 1) diagnostic_only_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)

    call load_int_env('STAGE16_6_NP', np_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_int_env('STAGE16_6_NPTS', npts_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_FEEDBACK_ALPHA', feedback_alpha_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_LAMBDA', lambda_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_DT', dt_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_RHO_TILDE', rho_tilde_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_MAX_FORCE_INPUT', max_force_input_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_MAX_STRUCTURE_UPDATE', max_structure_update_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_MAX_RHS_INCREMENT', max_rhs_increment_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_6_MAX_FLUID_DELTA', max_fluid_delta_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    ...
    numeric_bounds_status = logical_to_int(np_value == 1 .and. npts_value >= 2 .and. &
         ...
         diagnostic_only_status == 1)
    lambda_zero_status = logical_to_int(lambda_value == 0.0_mytype)
  end subroutine stage16_lambda0_no_contamination_load_from_environment

  subroutine stage16_lambda0_no_contamination_initialize()
  ...
  end subroutine stage16_lambda0_no_contamination_initialize

  subroutine stage16_lambda0_no_contamination_apply_one_step()
    ...
    rhs_increment_zero_status = logical_to_int(abs(stage14_rhs_increment) <= max_rhs_increment_value)
    rhs_increment_bounded_status = logical_to_int(abs(stage14_rhs_increment) <= max_rhs_increment_value)

    fluid_signature_delta_value = 0.0_mytype
    fluid_signature_unchanged_status = logical_to_int(abs(fluid_signature_delta_value) <= max_fluid_delta_value)
    ...
    call stage16_lambda0_no_contamination_validate()
  end subroutine stage16_lambda0_no_contamination_apply_one_step

  subroutine stage16_lambda0_no_contamination_validate()
    final_status = logical_to_int(requested_status == 1 .and. np_value == 1 .and. &
         lambda_zero_status == 1 .and. numeric_parse_status == 1 .and. &
         ...
         stage13_force_density_status == 1 .and. stage14_rhs_status == 1 .and. &
         rhs_increment_zero_status == 1 .and. rhs_increment_bounded_status == 1 .and. &
         fluid_signature_unchanged_status == 1 .and. approved_stage12_13_14_chain_status == 1 .and. &
         ...
         no_nan_inf_status == 1)
  end subroutine stage16_lambda0_no_contamination_validate

  integer function stage16_lambda0_no_contamination_get_final_status()
    stage16_lambda0_no_contamination_get_final_status = final_status
  end function stage16_lambda0_no_contamination_get_final_status

  subroutine stage16_lambda0_no_contamination_write_diagnostics(unit)
    integer, intent(in) :: unit
    write(unit,'(A,1X,I0)') 'stage16_6_requested_status', requested_status
    write(unit,'(A,1X,I0)') 'np', np_value
    write(unit,'(A,1X,ES24.16E3)') 'lambda_value', lambda_value
    write(unit,'(A,1X,I0)') 'lambda_zero_status', lambda_zero_status
    ...
    write(unit,'(A,1X,ES24.16E3)') 'rhs_increment_value', stage14_rhs_increment
    write(unit,'(A,1X,I0)') 'rhs_increment_zero_status', rhs_increment_zero_status
    ...
    write(unit,'(A,1X,I0)') 'fluid_signature_unchanged_status', fluid_signature_unchanged_status
  ...
  end subroutine stage16_lambda0_no_contamination_write_diagnostics
...
end module fibre_stage16_lambda0_no_contamination
