#!/usr/bin/env python3
"""Stage 16.3 force-sign/action-reaction audit helper."""
...
def active_shell_has_rg_command(script: Path) -> bool:
    ...
    for raw in read_text(script).splitlines():
        ...
        if not line or line.startswith("#"):
            continue
        # Strip simple trailing comments; command detection must not treat quoted regex text as rg use.
...
def stage14_lambda_zero_gate_found(repo: Path) -> bool:
    ...
    # misclassified as a real Stage 14 lambda==0 hook-registration gate regression.
...
def old_stage13_5_production_name_reappeared(repo: Path) -> bool:
    ...
    # force-density candidate name is forbidden; negative audit text in Stage 15/16
    # scripts/docs must not be treated as a real regression.
    files = [
...
def stage16_2_closed_static_evidence_ok(repo: Path) -> bool:
    required = [
    ...
    cmake_ok = "add_executable(fibre_stage16_one_fibre_case_check" in read_text(repo / "src" / "CMakeLists.txt")
...
def main() -> int:
    ...
    parser.add_argument("--require-stage16-2", default="1")
    parser.add_argument("--accept-stage16-2-closed-evidence", default="1")
    ...
    dat = out_dir / "fibre_stage16_3_force_sign_audit.dat"
    reasons_file = out_dir / "stage16_3_force_sign_audit_reasons.tmp"
    data = parse_dat(dat)
    reasons: list[str] = []
    ...
    if args.build_status != "1":
        reasons.append("stage16_3_force_sign_audit_target_build_status_not_pass")
    if args.run_status != "1":
        reasons.append("stage16_3_force_sign_audit_target_run_status_not_pass")

    wrapper = repo / "stage16_checks" / "run_stage16_3_force_sign_audit.sh"
    doc = repo / "stage16_checks" / "stage16_3_force_sign_audit.md"
    helper = repo / "stage16_checks" / "assert_stage16_3_force_sign_audit.py"
    audit_src = repo / "src" / "fibre_stage16_force_sign_audit.f90"
    check_src = repo / "src" / "fibre_stage16_force_sign_audit_check.f90"
    cmake = repo / "src" / "CMakeLists.txt"
    for path in [wrapper, doc, helper, audit_src, check_src]:
        if not path.exists():
            reasons.append(f"missing_required_stage16_3_file_{path}")
    if "add_executable(fibre_stage16_force_sign_audit_check" not in read_text(cmake):
        reasons.append("missing_fibre_stage16_force_sign_audit_check_build_registration")
    ...
    if args.require_stage16_2 == "1":
        stage16_2 = parse_dat(repo / "stage16_outputs" / "fibre_stage16_2_one_fibre_case_definition.dat")
        if stage16_2.get("final_status") != "1" and not (
            args.accept_stage16_2_closed_evidence == "1" and stage16_2_closed_static_evidence_ok(repo)
        ):
            reasons.append("missing_or_failed_stage16_2_case_definition_evidence")
    ...
    if any(script_has_rg_only_dependency(path) for path in all_files(repo / "stage15_checks", ("*.sh",)) + all_files(repo / "stage16_checks", ("*.sh",))):
    ...
    if "stage16" in read_text(repo / "src" / "xcompact3d.f90").lower():
        ...
        boundary_status = 0
    stage16_3_src = read_text(audit_src) + "\n" + read_text(check_src)
    ...
    for reason, pattern in forbidden_call_patterns.items():
        if re.search(pattern, stage16_3_src, re.I):
    ...
    with dat.open("w") as handle:
        handle.write("# Stage 16.3 force-sign/action-reaction summary\n")
        for key in REQUIRED_KEYS:
    ...
    if final_ok:
        print("STAGE 16.3 FORCE SIGN AUDIT VERDICT: PASS")
        print("STAGE 16.3 FINAL VERDICT: PASS")
        return 0
    print("STAGE 16.3 FORCE SIGN AUDIT VERDICT: FAIL")
    print("STAGE 16.3 FINAL VERDICT: FAIL")
    print("Reasons:")
    for reason in reasons:
        print(f" - {reason}")
    return 1
