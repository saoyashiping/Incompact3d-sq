program fibre_prod_hydro_input_candidate_check
  ...
  real(dp) :: sampled0(nnode, 3)
  real(dp) :: structure_u(nnode, 3)
  real(dp) :: expected_relative(nnode, 3)
  real(dp) :: expected_force(nnode, 3)
  real(dp) :: bad_structure_u(2, 3)
  integer :: status
  ...
  expected_relative = sampled0 - structure_u
  expected_force = beta * expected_relative
  ...
  if (maxval(abs(candidate%relative_u - expected_relative)) > tol) error stop 11
  if (maxval(abs(candidate%candidate_force - expected_force)) > tol) error stop 12
  if (abs(candidate%max_abs_relative_u - maxval(abs(expected_relative))) > tol) error stop 13
  if (abs(candidate%max_abs_candidate_force - maxval(abs(expected_force))) > tol) error stop 14
  if (abs(candidate%sum_abs_candidate_force - sum(abs(expected_force))) > tol) error stop 15
  if (.not. candidate%has_candidate) error stop 16

  call fibre_prod_hydro_input_candidate_attach_to_state(candidate, state, status)
  if (status /= 0) error stop 17
  if (.not. state%has_hydro_force_candidate) error stop 18
  if (maxval(abs(state%hydro_force_candidate - candidate%candidate_force)) > tol) error stop 19
  if (maxval(abs(state%x - x0)) > 0.0_dp) error stop 20
  if (maxval(abs(state%sampled_u - sampled0)) > 0.0_dp) error stop 21
  if (maxval(abs(state%v - v0)) > 0.0_dp) error stop 22
  if (maxval(abs(state%a - a0)) > 0.0_dp) error stop 23
  if (.not. same_field(ux, ux0) .or. .not. same_field(uy, uy0) .or. .not. same_field(uz, uz0)) error stop 24
  if (.not. same_field(rhs_x, rhs_x0) .or. .not. same_field(rhs_y, rhs_y0) .or. &
      .not. same_field(rhs_z, rhs_z0)) error stop 25
  ...
  if (any(force_buffer%fx /= 0.0_dp) .or. any(force_buffer%fy /= 0.0_dp) .or. &
      any(force_buffer%fz /= 0.0_dp)) error stop 27
  call fibre_prod_hydro_input_candidate_compute(candidate, state%sampled_u, structure_u, status)
  ...
  call fibre_prod_hydro_input_candidate_attach_to_state(candidate, state, status)
  if (status /= 0) error stop 42
  if (.not. same_field(rhs_x, rhs_x0) .or. .not. same_field(rhs_y, rhs_y0) .or. &
      .not. same_field(rhs_z, rhs_z0)) error stop 43
  if (any(force_buffer%fx /= 0.0_dp) .or. any(force_buffer%fy /= 0.0_dp) .or. &
      any(force_buffer%fz /= 0.0_dp)) error stop 44

  bad_structure_u = 0.0_dp
  call fibre_prod_hydro_input_candidate_compute(candidate, state%sampled_u, bad_structure_u, status)
  if (status == 0) error stop 45
  ...
  print *, 'P0_6_HYDRO_INPUT_CANDIDATE_CHECK PASS'
contains
...
end program fibre_prod_hydro_input_candidate_check
