module fibre_stage10_noop_hook
  ...
  integer, save :: stage10_3_write_status = 0
...
contains
  ...
  subroutine stage10_hook_finalize()
    finalize_status = 1
    call stage10_hook_write_stage10_3_dat_if_requested()
  end subroutine stage10_hook_finalize


  subroutine stage10_hook_write_stage10_3_dat_if_requested()
    character(len=16) :: env_value, rank_value
    integer :: env_status, rank_status, unit_id, io_status, main_status

    call get_environment_variable('OMPI_COMM_WORLD_RANK', value=rank_value, status=rank_status)
    if (rank_status == 0) then
      if (trim(adjustl(rank_value)) /= '0') return
    endif
    call get_environment_variable('PMI_RANK', value=rank_value, status=rank_status)
    if (rank_status == 0) then
      if (trim(adjustl(rank_value)) /= '0') return
    endif
    call get_environment_variable('SLURM_PROCID', value=rank_value, status=rank_status)
    if (rank_status == 0) then
      if (trim(adjustl(rank_value)) /= '0') return
    endif
    ...
    if (trim(adjustl(env_value)) /= '1') return

    if (requested_flag == 1 .and. noop_mode_status == 1 .and. initialized_status == 1 .and. &
        pre_step_status == 1 .and. pre_rhs_status == 1 .and. post_projection_status == 1 .and. &
        post_step_status == 1 .and. finalize_status == 1 .and. no_fibre_state_status == 1 .and. &
        no_force_status == 1 .and. no_rhs_injection_status == 1 .and. no_ibm_call_status == 1 .and. &
    ...
    write(unit_id,'(A,1X,I0)') 'stage10_3_field_modified_status', 0
    write(unit_id,'(A,1X,I0)') 'stage10_3_main_noop_hook_status', main_status
    close(unit_id)
    stage10_3_write_status = 1
  end subroutine stage10_hook_write_stage10_3_dat_if_requested
