program fibre_stage8_rk_sync_check
  ...
  type(stage8_lagrangian_state_t)::s,sblk; type(stage8_rk_sync_status_t)::st
  ...
  call init_stage8_rk_sync_status(st,3); sv=0; sr=0; bcount=0; err=0; sfd=0; ssu=0; ssf=0; ssd=0; bclr=0
  do r=1,3
    ...
    call apply_stage8_controlled_rk_substep_coupling(gbridge,layout,ux,uy,uz,beta,s,fx,fy,fz,st,r,ok,rej); sv=sv+ok; sr=sr+rej
  ...
  wf=merge(1,0,st%nsubstep==3.and.sv==3.and.sr==0.and.st%velocity_interpolation_count==3.and.st%slip_assembly_count==3.and.st%feedback_candidate_count==3.and.st%force_density_candidate_count==3.and.st%force_buffer_clear_count==3)
  frc=merge(1,0,sepfd>1e-12_mytype); stale=merge(1,0,sepu>1e-12_mytype.and.seps>1e-12_mytype.and.sepf>1e-12_mytype); bufs=merge(1,0,bcount>=2); ord=st%event_order_status; cons=merge(1,0,emax<=1e-12_mytype)
  ...
  call apply_stage8_controlled_rk_substep_coupling(gbridge,layout,ux,uy,uz,beta,sblk,fx,fy,fz,st,1,ok,rej); call compute_stage8_force_density_norm(fx,fy,fz,bnrm); blk=merge(1,0,rej==1.and.bnrm<=1e-14_mytype)
  norhs=merge(1,0,st%rhs_hook_called_flag==0.and.st%rhs_modified_flag==0.and.st%pressure_poisson_called_flag==0.and.st%projection_called_flag==0.and.st%production_dns_called_flag==0.and.st%fluid_update_called_flag==0.and.st%fibre_advance_called_flag==0)
  call init_rhs(rhs0); rhs1=rhs0; rhschg=maxval(abs(rhs1-rhs0)); noop=merge(1,0,rhschg<=1e-14_mytype)
  final=merge(1,0,dep==1.and.wf==1.and.frc==1.and.stale==1.and.bufs==1.and.ord==1.and.cons==1.and.blk==1.and.norhs==1.and.noop==1)
  ...
  write(io,'(A,1X,I0)') 'stage8_rk_nsubstep',st%nsubstep; write(io,'(A,1X,I0)') 'stage8_rk_substep_valid_count',sv; write(io,'(A,1X,I0)') 'stage8_rk_substep_rejected_count',sr; write(io,'(A,1X,I0)') 'stage8_rk_velocity_interpolation_count',st%velocity_interpolation_count; write(io,'(A,1X,I0)') 'stage8_rk_slip_assembly_count',st%slip_assembly_count; write(io,'(A,1X,I0)') 'stage8_rk_feedback_candidate_count',st%feedback_candidate_count; write(io,'(A,1X,I0)') 'stage8_rk_force_density_candidate_count',st%force_density_candidate_count; write(io,'(A,1X,I0)') 'stage8_rk_force_buffer_clear_count',st%force_buffer_clear_count; write(io,'(A,1X,I0)') 'stage8_rk_three_substep_workflow_status',wf
  ...
  write(io,'(A,1X,I0)') 'stage8_rk_clear_before_velocity_flag',merge(1,0,st%clear_buffer_event_index<st%velocity_event_index); write(io,'(A,1X,I0)') 'stage8_rk_velocity_before_slip_flag',merge(1,0,st%velocity_event_index<st%slip_event_index); write(io,'(A,1X,I0)') 'stage8_rk_slip_before_feedback_flag',merge(1,0,st%slip_event_index<st%feedback_event_index); write(io,'(A,1X,I0)') 'stage8_rk_feedback_before_force_density_flag',merge(1,0,st%feedback_event_index<st%force_density_event_index); write(io,'(A,1X,I0)') 'stage8_rk_force_density_before_rhs_gate_flag',merge(1,0,st%force_density_event_index<st%hypothetical_rhs_gate_event_index); write(io,'(A,1X,I0)') 'stage8_rk_rhs_gate_before_projection_gate_flag',merge(1,0,st%hypothetical_rhs_gate_event_index<st%hypothetical_projection_gate_event_index); write(io,'(A,1X,I0)') 'stage8_rk_event_order_status',ord
  ...
  write(io,'(A,1X,I0)') 'stage8_rk_blocked_substep_blocked_count',1; write(io,'(A,1X,I0)') 'stage8_rk_blocked_substep_rejected_flag',rej; write(io,'(A,1X,ES24.16E3)') 'stage8_rk_blocked_substep_force_buffer_norm_max',bnrm; write(io,'(A,1X,I0)') 'stage8_rk_blocked_substep_status',blk
  write(io,'(A,1X,I0)') 'stage8_rk_rhs_hook_called_flag',st%rhs_hook_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_rhs_modified_flag',st%rhs_modified_flag; write(io,'(A,1X,I0)') 'stage8_rk_pressure_poisson_modified_flag',st%pressure_poisson_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_projection_modified_flag',st%projection_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_real_projection_called_flag',0; write(io,'(A,1X,I0)') 'stage8_rk_production_dns_called_flag',st%production_dns_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_fluid_update_called_flag',st%fluid_update_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_fibre_advance_called_flag',st%fibre_advance_called_flag; write(io,'(A,1X,I0)') 'stage8_rk_no_rhs_no_projection_status',norhs
