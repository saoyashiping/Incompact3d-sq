...
contains
  ...
  subroutine stage11_production_oneway_finalize()
    integer :: io, ios
    call update_status()
    if (stage11_rank0()) then
       open(newunit=io,file='stage11_outputs/fibre_stage11_5_production_oneway_hook.dat', &
  ...
  logical function stage11_rank0()
    character(len=32) :: env
    integer :: status, rank_value, ios
    stage11_rank0=.true.
    call get_environment_variable('OMPI_COMM_WORLD_RANK',value=env,status=status)
    if (status/=0) call get_environment_variable('PMI_RANK',value=env,status=status)
    if (status/=0) call get_environment_variable('MPI_LOCALRANKID',value=env,status=status)
    if (status==0) then
       read(env,*,iostat=ios) rank_value
       if (ios==0) stage11_rank0=(rank_value==0)
    endif
  end function
