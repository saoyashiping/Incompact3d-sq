...
cmake --build build_stage6 --target fibre_stage6_config_check
if [[ -x build_stage6/bin/fibre_stage6_config_check ]]; then
  build_stage6/bin/fibre_stage6_config_check
else
  "$(find build_stage6 -type f -name fibre_stage6_config_check | head -n 1)"
fi
cat stage6_outputs/fibre_stage6_config_check.dat
