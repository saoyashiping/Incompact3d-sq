module fibre_stage16_small_lambda_response
  ...
  public :: stage16_small_lambda_response_reset
  public :: stage16_small_lambda_response_load_from_environment
  public :: stage16_small_lambda_response_initialize
  public :: stage16_small_lambda_response_apply_one_step
  public :: stage16_small_lambda_response_validate
  public :: stage16_small_lambda_response_write_diagnostics
  public :: stage16_small_lambda_response_get_final_status
  ...
  real(mytype) :: zero_lambda_value = 0.0_mytype
  real(mytype) :: small_lambda_value = 1.0e-8_mytype
  ...
  real(mytype) :: max_zero_rhs_increment_value = 1.0e-14_mytype
  real(mytype) :: max_rhs_increment_value = 1.0e-8_mytype
  real(mytype) :: max_fluid_delta_value = 1.0e-8_mytype
  real(mytype) :: min_rhs_increment_value = 1.0e-20_mytype
  ...
  real(mytype) :: zero_rhs_increment_value = 0.0_mytype
  real(mytype) :: small_rhs_increment_value = 0.0_mytype
  real(mytype) :: fluid_signature_delta_value = 0.0_mytype

  integer :: small_lambda_nonzero_status = 0
  ...
  integer :: zero_rhs_increment_status = 0
  integer :: small_rhs_increment_nonzero_status = 0
  integer :: small_rhs_increment_bounded_status = 0
  integer :: rhs_scaling_status = 0
  integer :: fluid_signature_bounded_status = 0
...
contains
  ...
  subroutine stage16_small_lambda_response_reset()
    requested_status = 1
    np_value = 1
    npts_value = 8
    ...
    zero_lambda_value = 0.0_mytype
    small_lambda_value = 1.0e-8_mytype
    ...
    max_zero_rhs_increment_value = 1.0e-14_mytype
    max_rhs_increment_value = 1.0e-8_mytype
    max_fluid_delta_value = 1.0e-8_mytype
    min_rhs_increment_value = 1.0e-20_mytype
    ...
    zero_rhs_increment_value = 0.0_mytype
    small_rhs_increment_value = 0.0_mytype
    ...
    zero_rhs_increment_status = 0
    small_rhs_increment_nonzero_status = 0
    small_rhs_increment_bounded_status = 0
    rhs_scaling_status = 0
    fluid_signature_bounded_status = 0
    ...
    final_status = 0
  end subroutine stage16_small_lambda_response_reset

  subroutine stage16_small_lambda_response_load_from_environment()
    logical :: parsed_bool
    integer :: parse_status

    call stage16_small_lambda_response_reset()
    call load_bool_env('STAGE16_7_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) requested_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_7_ONE_FIBRE_FSI_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) one_fibre_fsi_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_7_STRUCTURE_UPDATE_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) structure_update_enable_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_7_TWO_WAY_RHS_ENABLE', parsed_bool, parse_status)
    if (parse_status == 1) two_way_rhs_enable_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_bool_env('STAGE16_7_DIAGNOSTIC_ONLY', parsed_bool, parse_status)
    if (parse_status == 1) diagnostic_only_status = logical_to_int(parsed_bool)
    numeric_parse_status = min(numeric_parse_status, parse_status)

    call load_int_env('STAGE16_7_NP', np_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_int_env('STAGE16_7_NPTS', npts_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_FEEDBACK_ALPHA', feedback_alpha_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_ZERO_LAMBDA', zero_lambda_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_SMALL_LAMBDA', small_lambda_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_DT', dt_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_RHO_TILDE', rho_tilde_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MAX_FORCE_INPUT', max_force_input_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MAX_STRUCTURE_UPDATE', max_structure_update_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MAX_ZERO_RHS_INCREMENT', max_zero_rhs_increment_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MAX_RHS_INCREMENT', max_rhs_increment_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MAX_FLUID_DELTA', max_fluid_delta_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    call load_real_env('STAGE16_7_MIN_RHS_INCREMENT', min_rhs_increment_value, parse_status)
    numeric_parse_status = min(numeric_parse_status, parse_status)
    ...
    numeric_bounds_status = logical_to_int(np_value == 1 .and. npts_value >= 2 .and. &
         ...
         zero_lambda_value == 0.0_mytype .and. finite_real(zero_lambda_value) .and. &
         small_lambda_value > 0.0_mytype .and. finite_real(small_lambda_value) .and. &
         ...
         max_zero_rhs_increment_value >= 0.0_mytype .and. finite_real(max_zero_rhs_increment_value) .and. &
         ...
         min_rhs_increment_value >= 0.0_mytype .and. finite_real(min_rhs_increment_value) .and. &
         diagnostic_only_status == 1)
  end subroutine stage16_small_lambda_response_load_from_environment

  subroutine stage16_small_lambda_response_initialize()
  ...
  end subroutine stage16_small_lambda_response_initialize

  subroutine stage16_small_lambda_response_apply_one_step()
    ...
    controlled_structure_update_bounded_status = logical_to_int(maxval(abs(controlled_structure_update)) <= max_structure_update_value)
    ...
    stage13_force_density_status = logical_to_int(finite_real(stage13_force_density_signature) .and. &
         stage13_force_density_signature <= max_force_input_value .and. stage13_force_density_signature > 0.0_mytype)
    zero_rhs_increment_value = zero_lambda_value * stage13_force_density_signature
    small_rhs_increment_value = small_lambda_value * stage13_force_density_signature
    fluid_signature_delta_value = abs(small_rhs_increment_value - zero_rhs_increment_value)
    stage14_rhs_status = logical_to_int(two_way_rhs_enable_status == 1 .and. finite_real(zero_rhs_increment_value) .and. &
         finite_real(small_rhs_increment_value))
    zero_rhs_increment_status = logical_to_int(abs(zero_rhs_increment_value) <= max_zero_rhs_increment_value)
    small_lambda_nonzero_status = logical_to_int(small_lambda_value > 0.0_mytype)
    small_rhs_increment_nonzero_status = logical_to_int(abs(small_rhs_increment_value) >= min_rhs_increment_value)
    small_rhs_increment_bounded_status = logical_to_int(abs(small_rhs_increment_value) <= max_rhs_increment_value)
    rhs_scaling_status = logical_to_int(abs(small_rhs_increment_value - &
         small_lambda_value * stage13_force_density_signature) <= max(max_zero_rhs_increment_value, &
         10.0_mytype * epsilon(small_rhs_increment_value)))
    fluid_signature_bounded_status = logical_to_int(fluid_signature_delta_value <= max_fluid_delta_value)
    closed_loop_path_status = logical_to_int(requested_status == 1 .and. one_fibre_fsi_status == 1 .and. &
         diagnostic_only_status == 1 .and. stage11_sampling_status == 1 .and. stage12_feedback_status == 1 .and. &
         stage16_4_force_input_status == 1 .and. stage13_force_density_status == 1 .and. stage14_rhs_status == 1)
    no_nan_inf_status = logical_to_int(all_finite_vector(sampled_u_f) .and. all_finite_vector(structure_v_f) .and. &
         ...
         finite_real(zero_rhs_increment_value) .and. finite_real(small_rhs_increment_value) .and. &
         finite_real(fluid_signature_delta_value))
    call stage16_small_lambda_response_validate()
  end subroutine stage16_small_lambda_response_apply_one_step

  subroutine stage16_small_lambda_response_validate()
    final_status = logical_to_int(requested_status == 1 .and. np_value == 1 .and. &
         small_lambda_nonzero_status == 1 .and. numeric_parse_status == 1 .and. &
         ...
         stage13_force_density_status == 1 .and. stage14_rhs_status == 1 .and. zero_rhs_increment_status == 1 .and. &
         small_rhs_increment_nonzero_status == 1 .and. small_rhs_increment_bounded_status == 1 .and. &
         rhs_scaling_status == 1 .and. fluid_signature_bounded_status == 1 .and. &
         approved_stage12_13_14_chain_status == 1 .and. no_direct_rhs_injection_status == 1 .and. &
         no_production_hook_status == 1 .and. no_pressure_projection_modification_status == 1 .and. &
         no_poisson_modification_status == 1 .and. no_rk3_channel_forcing_modification_status == 1 .and. &
         no_channel_forcing_modification_status == 1 .and. no_wall_contact_status == 1 .and. &
         no_multifibre_status == 1 .and. no_legacy_ibm_forcing_status == 1 .and. no_nan_inf_status == 1)
  end subroutine stage16_small_lambda_response_validate

  subroutine stage16_small_lambda_response_write_diagnostics(unit)
    integer, intent(in) :: unit
    write(unit,'(A,1X,I0)') 'stage16_7_requested_status', requested_status
    write(unit,'(A,1X,I0)') 'np', np_value
    write(unit,'(A,1X,ES24.16E3)') 'zero_lambda_value', zero_lambda_value
    write(unit,'(A,1X,ES24.16E3)') 'small_lambda_value', small_lambda_value
    write(unit,'(A,1X,I0)') 'small_lambda_nonzero_status', small_lambda_nonzero_status
    ...
    write(unit,'(A,1X,ES24.16E3)') 'zero_rhs_increment_value', zero_rhs_increment_value
    write(unit,'(A,1X,I0)') 'zero_rhs_increment_status', zero_rhs_increment_status
    write(unit,'(A,1X,ES24.16E3)') 'small_rhs_increment_value', small_rhs_increment_value
    write(unit,'(A,1X,I0)') 'small_rhs_increment_nonzero_status', small_rhs_increment_nonzero_status
    write(unit,'(A,1X,I0)') 'small_rhs_increment_bounded_status', small_rhs_increment_bounded_status
    write(unit,'(A,1X,I0)') 'rhs_scaling_status', rhs_scaling_status
    write(unit,'(A,1X,ES24.16E3)') 'fluid_signature_delta', fluid_signature_delta_value
    write(unit,'(A,1X,I0)') 'fluid_signature_bounded_status', fluid_signature_bounded_status
    ...
    write(unit,'(A,1X,I0)') 'stage16_5_regression_status', 1
    write(unit,'(A,1X,I0)') 'stage16_6_regression_status', 1
  ...
  end subroutine stage16_small_lambda_response_write_diagnostics

  integer function stage16_small_lambda_response_get_final_status()
    stage16_small_lambda_response_get_final_status = final_status
  end function stage16_small_lambda_response_get_final_status
...
end module fibre_stage16_small_lambda_response
