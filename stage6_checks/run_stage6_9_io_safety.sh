...
cmake --build build_stage6 --target fibre_stage6_io_safety_check
EXE="build_stage6/bin/fibre_stage6_io_safety_check"
if [[ ! -x "$EXE" ]]; then
  EXE=$(find build_stage6 -type f -name fibre_stage6_io_safety_check | head -n 1)
fi
"$EXE"
cat stage6_outputs/fibre_stage6_io_safety_check.dat
