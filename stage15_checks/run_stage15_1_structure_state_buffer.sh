...
scan_stage15_1_source_guards() {
    ...
    if search_report "use[[:space:]]+fibre_stage12|stage12_|feedback_force|feedback_candidate" $files >> "$STATIC_REPORT" 2>/dev/null; then
    ...
    if search_report "use[[:space:]]+fibre_stage14|stage14_|rhs_injection|injection_gain" $files >> "$STATIC_REPORT" 2>/dev/null; then
    ...
    if search_report "fluid_rhs[[:space:]]*[(=]|(^|[^_[:alnum:]])(rhs|ux|uy|uz|gx|gy|gz)[[:space:]]*[(=]" $files >> "$STATIC_REPORT" 2>/dev/null; then
...
run_static_audit() {
    ...
    if search_report "fibre_stage15_structure_state_check" src/CMakeLists.txt >/dev/null 2>&1; then
        DOC_STATUS=1
    else
    ...
    if search_report 'stage14_get_injection_gain[[:space:]]*\([[:space:]]*\)[[:space:]]*==[[:space:]]*0\.0(_[[:alnum:]_]+)?' src/xcompact3d.f90 >> "$STATIC_REPORT" 2>/dev/null; then
