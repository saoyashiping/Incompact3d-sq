program fibre_stage8_runtime_grid_bridge_check
  ...
  implicit none
  type(stage7_channel_grid_t) :: gref,gbridge_good,gbad,greal
  type(stage8_runtime_grid_bridge_status_t) :: st_good,st_bad,streal
  ...
  integer :: dy_positive_flag,volume_positive_flag,wall_bounds_status,grid_detail_status
  real(mytype) :: total_ref,total_bridge
  ...
  call init_stage8_runtime_bridge_status(st_good)
  call init_stage8_grid_from_explicit_arrays_bridge(gbridge_good,nx,ny,nz,gref%xmin,gref%xmax,gref%zmin,gref%zmax,yface,ycenter,1,1,st_good)

  err_meta=max(abs(gbridge_good%dx-gref%dx),abs(gbridge_good%dz-gref%dz))
  err_meta=max(err_meta,maxval(abs(gbridge_good%y_face-gref%y_face)))
  err_meta=max(err_meta,maxval(abs(gbridge_good%y_center-gref%y_center)))
  err_meta=max(err_meta,maxval(abs(gbridge_good%dy_cell-gref%dy_cell)))
  err_meta=max(err_meta,maxval(abs(gbridge_good%volume_y-gref%volume_y)))
  call compute_stage7_total_volume(gref,total_ref)
  call compute_stage7_total_volume(gbridge_good,total_bridge)
  err_meta=max(err_meta,abs(total_bridge-total_ref))
  ...
  real_ev_status=merge(1,0,rep_exists==1 .and. st_good%explicit_array_fallback_called_flag==1 .and. &
  ...
  err_dy=maxval(abs((gbridge_good%y_face(2:ny+1)-gbridge_good%y_face(1:ny))-gbridge_good%dy_cell))
  err_dy=max(err_dy,gbridge_good%dy_face_consistency_error_max)
  err_vol=maxval(abs(gbridge_good%volume_y-gbridge_good%dx*gbridge_good%dy_cell*gbridge_good%dz))
  err_vol=max(err_vol,gbridge_good%volume_formula_error_max)

  dy_positive_flag=merge(1,0,minval(gbridge_good%dy_cell)>0.0_mytype)
  volume_positive_flag=merge(1,0,minval(gbridge_good%volume_y)>0.0_mytype)
  wall_bounds_status=merge(1,0,abs(gbridge_good%y_face(1)-gbridge_good%ymin)<=tol .and. &
       abs(gbridge_good%y_face(gbridge_good%ny+1)-gbridge_good%ymax)<=tol .and. gbridge_good%periodic_y==0)
  grid_detail_status=merge(1,0,gbridge_good%x_uniform_flag==1 .and. gbridge_good%z_uniform_flag==1 .and. &
       gbridge_good%y_nonuniform_flag==1 .and. gbridge_good%y_monotonic_flag==1 .and. &
       dy_positive_flag==1 .and. volume_positive_flag==1 .and. err_dy<=tol .and. err_vol<=tol .and. &
       wall_bounds_status==1)
  ...
  call build_stage7_scalar_interp_weight(gbridge_good,gbridge_good%xmin+3*gbridge_good%dx,gbridge_good%y_center(8),gbridge_good%zmin+3*gbridge_good%dz,w)
  ...
  call interpolate_stage7_velocity(gbridge_good,layout,ux,uy,uz,gbridge_good%xmin+3*gbridge_good%dx,gbridge_good%y_center(8),gbridge_good%zmin+3*gbridge_good%dz,ulag,v,r)
  ...
  call spread_stage7_lagrangian_force(gbridge_good,gbridge_good%xmin+3*gbridge_good%dx,gbridge_good%y_center(8),gbridge_good%zmin+3*gbridge_good%dz,[1._mytype,0._mytype,0._mytype],0.1_mytype,fx,fy,fz,v,r)
  call compute_stage7_eulerian_force_total(gbridge_good,fx,fy,fz,fe); call compute_stage7_lagrangian_force_total(1,reshape([1._mytype,0._mytype,0._mytype],[3,1]),[0.1_mytype],fl)
  spread_ok=merge(1,0,v==1 .and. sqrt(sum((fe-fl)**2))<=1e-12_mytype)

  call classify_stage7_boundary_point(gbridge_good,layout,gbridge_good%xmin+3*gbridge_good%dx,gbridge_good%y_center(8),gbridge_good%zmin+3*gbridge_good%dz,1,bres)
  ...
  yface(8)=yface(7); call init_stage8_runtime_bridge_status(st_bad); call init_stage8_grid_from_explicit_arrays_bridge(gbad,nx,ny,nz,gref%xmin,gref%xmax,gref%zmin,gref%zmax,yface,ycenter,1,1,st_bad); inv_yface=merge(1,0,st_bad%bridge_grid_rejected_flag==1)
  yface=gref%y_face; yface(9)=yface(8)-1e-3_mytype; call init_stage8_runtime_bridge_status(st_bad); call init_stage8_grid_from_explicit_arrays_bridge(gbad,nx,ny,nz,gref%xmin,gref%xmax,gref%zmin,gref%zmax,yface,ycenter,1,1,st_bad); inv_dy=merge(1,0,st_bad%bridge_grid_rejected_flag==1)
  yface=gref%y_face; call init_stage8_runtime_bridge_status(st_bad); call init_stage8_grid_from_explicit_arrays_bridge(gbad,nx,ny,nz,gref%xmin,gref%xmax,gref%zmin,gref%zmax,yface,ycenter,0,1,st_bad); inv_px=merge(1,0,st_bad%bridge_grid_rejected_flag==1)
  call init_stage8_runtime_bridge_status(st_bad); call init_stage8_grid_from_explicit_arrays_bridge(gbad,nx,ny,nz,gref%xmin,gref%xmax,gref%zmin,gref%zmax,yface,ycenter,1,0,st_bad); inv_pz=merge(1,0,st_bad%bridge_grid_rejected_flag==1)
  ...
  final_status=merge(1,0,dep==1 .and. st_good%bridge_status==1 .and. st_good%explicit_array_fallback_called_flag==1 .and. &
       st_good%validate_stage7_grid_called_flag==1 .and. st_good%bridge_grid_valid_flag==1 .and. &
       st_good%bridge_grid_rejected_flag==0 .and. err_meta<=tol .and. real_ev_status==1 .and. &
       grid_detail_status==1 .and. iface_ok==1 .and. inv_status==1 .and. noop_ok==1)
  ...
  write(io,'(A,1X,I0)') 'stage8_bridge_explicit_array_fallback_called_flag',st_good%explicit_array_fallback_called_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_validate_stage7_grid_called_flag',st_good%validate_stage7_grid_called_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_explicit_grid_valid_flag',st_good%bridge_grid_valid_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_explicit_grid_rejected_flag',st_good%bridge_grid_rejected_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_explicit_bridge_status',st_good%bridge_status
  ...
  write(io,'(A,1X,I0)') 'stage8_bridge_x_uniform_flag',gbridge_good%x_uniform_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_z_uniform_flag',gbridge_good%z_uniform_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_y_nonuniform_flag',gbridge_good%y_nonuniform_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_y_monotonic_flag',gbridge_good%y_monotonic_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_dy_positive_flag',dy_positive_flag
  write(io,'(A,1X,I0)') 'stage8_bridge_volume_positive_flag',volume_positive_flag
  ...
  write(io,'(A,1X,I0)') 'stage8_bridge_wall_bounds_status',wall_bounds_status
  write(io,'(A,1X,I0)') 'stage8_bridge_grid_validation_detail_status',grid_detail_status
