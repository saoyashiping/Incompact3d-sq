...
program xcompact3d
  ...
  use fibre_stage15_production_structure_hook, only : &
       stage15_production_structure_hook_reset, &
  ...
  use fibre_prod_structure_commit_gate, only : fibre_prod_structure_commit_gate_env_enabled, &
       fibre_prod_structure_commit_gate_runtime_diagnostic
  ...
  use fibre_prod_p1_twoway_channel_case, only : &
       fibre_prod_p1_twoway_channel_case_env_enabled, &
       fibre_prod_p1_twoway_channel_case_init, &
       fibre_prod_p1_twoway_channel_case_record_sampling, &
       fibre_prod_p1_twoway_channel_case_record_structure_response, &
       fibre_prod_p1_twoway_channel_case_record_reaction_force, &
       fibre_prod_p1_twoway_channel_case_record_force_buffer, &
       fibre_prod_p1_twoway_channel_case_record_rhs_increment, &
       fibre_prod_p1_twoway_channel_case_write_diagnostics
  use fibre_prod_p1_enlarged_stability_case, only : &
       fibre_prod_p1_enlarged_stability_case_env_enabled, &
       fibre_prod_p1_enlarged_stability_case_init, &
       fibre_prod_p1_enlarged_stability_case_record_sampling, &
       fibre_prod_p1_enlarged_stability_case_record_structure_response, &
       fibre_prod_p1_enlarged_stability_case_record_reaction_force, &
       fibre_prod_p1_enlarged_stability_case_record_force_buffer, &
  ...
  use fibre_prod_p1_np_consistency_closure_case, only : &
       p1_4_np_env_enabled, &
       p1_4_np_init, &
       p1_4_np_record_sampling, &
       p1_4_np_record_structure_response, &
       p1_4_np_record_reaction_force, &
       p1_4_np_record_force_buffer, &
       p1_4_np_record_rhs_increment, &
       p1_4_np_record_global_signatures, &
       p1_4_np_write_diagnostics
  ...
  logical :: fibre_prod_structure_commit_gate_enabled
  ...
  stage15_structure_hook_reg = stage15_requested() .and. stage15_diagnostic_only() .and. &
       (.not. stage15_structure_advance_enabled())
  if (stage15_structure_hook_reg) then
  ...
  fibre_prod_structure_commit_gate_enabled = fibre_prod_structure_commit_gate_env_enabled()
  ...
  fibre_prod_p1_np_consistency_closure_case_enabled = p1_4_np_env_enabled()
  ...
  if (fibre_prod_p1_twoway_channel_case_enabled) then
     call fibre_prod_p1_twoway_channel_case_init(fibre_prod_p1_twoway_channel_case_status)
     if (fibre_prod_p1_twoway_channel_case_status /= 0) then
        if (nrank == 0) write(*,'(A,I0)') &
             'P1_2 two-way channel case fail-closed init status=', &
             fibre_prod_p1_twoway_channel_case_status
        call finalise_xcompact3d()
        stop 96
     endif
  endif
  if (fibre_prod_p1_enlarged_stability_case_enabled) then
     call fibre_prod_p1_enlarged_stability_case_init(fibre_prod_p1_enlarged_stability_case_status)
     if (fibre_prod_p1_enlarged_stability_case_status /= 0) then
        if (nrank == 0) write(*,'(A,I0)') &
             'P1_3 enlarged stability case fail-closed init status=', &
  ...
  if (fibre_prod_p1_np_consistency_closure_case_enabled) then
     call p1_4_np_init(fibre_prod_p1_np_consistency_closure_case_status)
  ...
  do itime=ifirst,ilast
     ...
     do itr=1,iadvance_time
        ...
        if (fibre_prod_structure_commit_gate_enabled) then
           call fibre_prod_structure_commit_gate_runtime_diagnostic(ux1,uy1,uz1, fibre_prod_structure_commit_gate_status)
           if (fibre_prod_structure_commit_gate_status /= 0) fibre_prod_structure_commit_gate_enabled = .false.
        endif
        if (fibre_prod_reaction_force_candidate_enabled .and. fibre_prod_reaction_spreading_enabled) then
           call fibre_prod_reaction_spreading_buffer_runtime_diagnostic(ux1,uy1,uz1, fibre_prod_reaction_spreading_status)
           if (fibre_prod_reaction_spreading_status /= 0) fibre_prod_reaction_spreading_enabled = .false.
        endif
        if (fibre_prod_reaction_force_candidate_enabled .and. fibre_prod_reaction_spreading_enabled .and. &
             fibre_prod_force_buffer_rhs_gate_enabled) then
        ...
        if (fibre_prod_p1_twoway_channel_case_enabled) then
           call fibre_prod_p1_twoway_channel_case_record_sampling(ux1,uy1,uz1, &
                fibre_prod_p1_twoway_channel_case_status)
           if (fibre_prod_p1_twoway_channel_case_status == 0) &
                call fibre_prod_p1_twoway_channel_case_record_structure_response( &
                fibre_prod_p1_twoway_channel_case_status)
           if (fibre_prod_p1_twoway_channel_case_status == 0) &
                call fibre_prod_p1_twoway_channel_case_record_reaction_force( &
                fibre_prod_p1_twoway_channel_case_status)
           if (fibre_prod_p1_twoway_channel_case_status == 0) &
                call fibre_prod_p1_twoway_channel_case_record_force_buffer( &
                fibre_prod_p1_twoway_channel_case_status)
           if (fibre_prod_p1_twoway_channel_case_status == 0) &
                call fibre_prod_p1_twoway_channel_case_record_rhs_increment( &
                dux1(:,:,:,1),duy1(:,:,:,1),duz1(:,:,:,1), &
                fibre_prod_p1_twoway_channel_case_status)
           if (fibre_prod_p1_twoway_channel_case_status /= 0) then
              if (nrank == 0) write(*,'(A,I0)') &
                   'P1_2 two-way channel case fail-closed runtime status=', &
                   fibre_prod_p1_twoway_channel_case_status
              call finalise_xcompact3d()
              stop 97
           endif
        ...
        if (fibre_prod_p1_enlarged_stability_case_enabled) then
           call fibre_prod_p1_enlarged_stability_case_record_sampling(ux1,uy1,uz1, &
                fibre_prod_p1_enlarged_stability_case_status)
           if (fibre_prod_p1_enlarged_stability_case_status == 0) &
                call fibre_prod_p1_enlarged_stability_case_record_structure_response( &
                fibre_prod_p1_enlarged_stability_case_status)
           if (fibre_prod_p1_enlarged_stability_case_status == 0) &
                call fibre_prod_p1_enlarged_stability_case_record_reaction_force( &
                fibre_prod_p1_enlarged_stability_case_status)
           if (fibre_prod_p1_enlarged_stability_case_status == 0) &
                call fibre_prod_p1_enlarged_stability_case_record_force_buffer( &
                fibre_prod_p1_enlarged_stability_case_status)
           if (fibre_prod_p1_enlarged_stability_case_status == 0) &
                call fibre_prod_p1_enlarged_stability_case_record_rhs_increment( &
                dux1(:,:,:,1),duy1(:,:,:,1),duz1(:,:,:,1), &
                fibre_prod_p1_enlarged_stability_case_status)
           if (fibre_prod_p1_enlarged_stability_case_status /= 0) then
              if (nrank == 0) write(*,'(A,I0)') &
                   'P1_3 enlarged stability case fail-closed runtime status=', &
                   fibre_prod_p1_enlarged_stability_case_status
              call finalise_xcompact3d()
              stop 100
           endif
        ...
        if (fibre_prod_p1_np_consistency_closure_case_enabled) then
           call p1_4_np_record_sampling(ux1,uy1,uz1, &
                fibre_prod_p1_np_consistency_closure_case_status)
           if (fibre_prod_p1_np_consistency_closure_case_status == 0) &
                call p1_4_np_record_structure_response( &
                fibre_prod_p1_np_consistency_closure_case_status)
           if (fibre_prod_p1_np_consistency_closure_case_status == 0) &
                call p1_4_np_record_reaction_force( &
                fibre_prod_p1_np_consistency_closure_case_status)
           if (fibre_prod_p1_np_consistency_closure_case_status == 0) &
                call p1_4_np_record_force_buffer( &
                fibre_prod_p1_np_consistency_closure_case_status)
           if (fibre_prod_p1_np_consistency_closure_case_status == 0) &
                call p1_4_np_record_rhs_increment( &
                dux1(:,:,:,1),duy1(:,:,:,1),duz1(:,:,:,1), &
                fibre_prod_p1_np_consistency_closure_case_status)
           if (fibre_prod_p1_np_consistency_closure_case_status /= 0) then
              if (nrank == 0) write(*,'(A,I0)') &
     ...
     if (stage15_structure_hook_reg) call stage15_production_structure_hook_apply(itime, 0)
  ...
  if (fibre_prod_p1_twoway_channel_case_enabled) then
     call fibre_prod_p1_twoway_channel_case_write_diagnostics(fibre_prod_p1_twoway_channel_case_status)
     if (fibre_prod_p1_twoway_channel_case_status /= 0) stop 98
  endif
  if (fibre_prod_p1_enlarged_stability_case_enabled) then
  ...
  if (fibre_prod_p1_np_consistency_closure_case_enabled) then
     call p1_4_np_record_global_signatures(fibre_prod_p1_np_consistency_closure_case_status)
     call p1_4_np_write_diagnostics(fibre_prod_p1_np_consistency_closure_case_status)
