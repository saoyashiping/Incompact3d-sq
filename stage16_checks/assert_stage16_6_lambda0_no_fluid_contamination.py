#!/usr/bin/env python3
"""Stage 16.6 lambda=0 no-fluid-contamination audit helper."""
...
NUMERIC_KEYS = {"np", "lambda_value", "rhs_increment_value", "fluid_signature_delta"}
...
def check_rg_fallback(script: Path) -> bool:
    ...
    This deliberately mirrors the corrected Stage 16.5 / Stage 16.4 false-positive-safe audit style:
...
def add_static_audit_reasons(repo: Path, data: dict[str, str], reasons: list[str]) -> None:
    """Apply Stage 16.6 static checks without broad false-positive-prone scans.

    False-positive protections are intentional and required for Stage 16.6:
    ...
    * if a forbidden pattern cannot be distinguished from documentation/negative-check text, this
    ...
    for idx, stem in STAGE15_DOC_SCRIPT_STEMS.items():
        ...
        if not doc.exists():
            ...
            stage15_regression_status = 0
    for script in stage15_shell_files + [repo / "stage16_checks" / "run_stage16_6_lambda0_no_fluid_contamination.sh"]:
    ...
    stage16_5_regression_status = check_stage16_evidence(repo, reasons, 5, [
    ...
    ], "add_executable(fibre_stage16_closed_loop_dryrun_check")

    boundary_status = 1
    if "stage16" in read_text(repo / "src" / "xcompact3d.f90").lower():
        reasons.append("stage16_production_hook_detected_in_xcompact3d")
        boundary_status = 0
    stage16_6_src = read_text(repo / "src" / "fibre_stage16_lambda0_no_contamination.f90") + "\n" + \
        read_text(repo / "src" / "fibre_stage16_lambda0_no_contamination_check.f90")
    ...
    for reason, pattern in forbidden_call_patterns.items():
        if re.search(pattern, stage16_6_src, re.I):
    ...
    data["stage16_5_regression_status"] = str(stage16_5_regression_status)
...
def main() -> int:
    ...
    parser.add_argument("--require-stage16-5", default="1")
    parser.add_argument("--accept-stage16-5-closed-evidence", default="1")
    ...
    dat = out_dir / "fibre_stage16_6_lambda0_no_fluid_contamination.dat"
    reasons_file = out_dir / "stage16_6_lambda0_no_fluid_contamination_reasons.tmp"
    data = parse_dat(dat)
    reasons: list[str] = []
    ...
    if args.build_status != "1":
        reasons.append("stage16_6_lambda0_no_contamination_target_build_status_not_pass")
    if args.run_status != "1":
        reasons.append("stage16_6_lambda0_no_contamination_target_run_status_not_pass")
    ...
    for path in required_files:
        if not path.exists():
            reasons.append(f"missing_required_stage16_6_file_{path}")
    if "add_executable(fibre_stage16_lambda0_no_contamination_check" not in read_text(repo / "src" / "CMakeLists.txt"):
        reasons.append("missing_fibre_stage16_lambda0_no_contamination_check_build_registration")
    ...
    if args.require_stage16_5 == "1":
        stage16_5 = parse_dat(repo / "stage16_outputs" / "fibre_stage16_5_closed_loop_dryrun_np1.dat")
        stage16_5_files_ok = all(path.exists() for path in stage16_5_files)
        if stage16_5.get("final_status") != "1" and not (args.accept_stage16_5_closed_evidence == "1" and stage16_5_files_ok):
            reasons.append("missing_or_failed_stage16_5_closed_loop_dryrun_evidence")
    ...
    try:
        if abs(float(data.get("lambda_value", "nan"))) > 0.0:
            reasons.append("lambda_value_not_zero")
    except ValueError:
        reasons.append("lambda_value_not_numeric")
    try:
        if abs(float(data.get("rhs_increment_value", "nan"))) > 1.0e-14:
            reasons.append("rhs_increment_value_exceeds_lambda0_bound")
    except ValueError:
        reasons.append("rhs_increment_value_not_numeric")
    try:
        if abs(float(data.get("fluid_signature_delta", "nan"))) > 1.0e-14:
    ...
    with dat.open("w") as handle:
        handle.write("# Stage 16.6 np=1 lambda=0 no-fluid-contamination summary\n")
        for key in REQUIRED_KEYS:
            handle.write(f"{key} {data.get(key, '0')}\n")
        for key in ["numeric_parse_status", "numeric_bounds_status", "feedback_alpha",
    ...
    if final_ok:
        print("STAGE 16.6 LAMBDA0 NO-FLUID-CONTAMINATION VERDICT: PASS")
        print("STAGE 16.6 FINAL VERDICT: PASS")
        return 0
    print("STAGE 16.6 LAMBDA0 NO-FLUID-CONTAMINATION VERDICT: FAIL")
    print("STAGE 16.6 FINAL VERDICT: FAIL")
    print("Reasons:")
    for reason in reasons:
        print(f" - {reason}")
    return 1
