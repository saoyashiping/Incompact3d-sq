module fibre_prod_p1_twoway_channel_case
  use, intrinsic :: iso_fortran_env, only : real64
  use, intrinsic :: ieee_arithmetic, only : ieee_is_finite
  implicit none
  private
  public :: fibre_prod_p1_twoway_channel_case_env_enabled
  public :: fibre_prod_p1_twoway_channel_case_init
  public :: fibre_prod_p1_twoway_channel_case_init_values
  public :: fibre_prod_p1_twoway_channel_case_record_sampling
  public :: fibre_prod_p1_twoway_channel_case_record_structure_response
  public :: fibre_prod_p1_twoway_channel_case_record_reaction_force
  public :: fibre_prod_p1_twoway_channel_case_record_force_buffer
  public :: fibre_prod_p1_twoway_channel_case_record_rhs_increment
  public :: fibre_prod_p1_twoway_channel_case_check_wall_safety
  public :: fibre_prod_p1_twoway_channel_case_check_rhs_scaling
  public :: fibre_prod_p1_twoway_channel_case_write_diagnostics
  public :: fibre_prod_p1_twoway_channel_case_get_rhs_norm
  public :: fibre_prod_p1_twoway_channel_case_set_sample_for_check

  integer :: fibre_count=1, fibre_nnode=49, sample_count=0, response_count=0
  real(real64) :: lambda_fsi=1.0e-5_real64, penalty_beta=2.0_real64, wall_clearance_min=0.10_real64
...
contains
  logical function fibre_prod_p1_twoway_channel_case_env_enabled()
    fibre_prod_p1_twoway_channel_case_env_enabled = read_env_logical('FIBRE_PROD_P1_TWOWAY_CHANNEL_ENABLE', .false.)
  end function

  subroutine fibre_prod_p1_twoway_channel_case_init(status)
    integer, intent(out) :: status
    call fibre_prod_p1_twoway_channel_case_init_values(read_env_int('FIBRE_PROD_P1_FIBRE_COUNT',1), &
      read_env_int('FIBRE_PROD_P1_FIBRE_NNODE',49), read_env_real('FIBRE_PROD_LAMBDA',1.0e-5_real64), &
      read_env_real('FIBRE_PROD_PENALTY_BETA',2.0_real64), &
      read_env_real('FIBRE_PROD_P1_WALL_CLEARANCE_MIN',0.10_real64), &
      read_env_real('FIBRE_PROD_P1_TWOWAY_MAX_DX',1.0e-2_real64), &
      read_env_real('FIBRE_PROD_P1_TWOWAY_MAX_STRUCTURE_U',1.0e2_real64), &
      read_env_real('FIBRE_PROD_P1_TWOWAY_MAX_RHS_INCREMENT',1.0e2_real64), &
      read_env_real('FIBRE_PROD_P1_TWOWAY_MAX_FORCE_BUFFER',1.0e6_real64), status)
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_init_values(count_in,nnode_in,lambda_in,beta_in,clearance_in, &
    ...
    initialized=.true.; call fibre_prod_p1_twoway_channel_case_check_wall_safety(status)
    if (status /= 0) return
    write(*,'(A,ES12.4)') 'P1_2 fibre initialization diagnostic: count=1 nnode=49 inside_channel=PASS lambda=', lambda_fsi
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_record_sampling(ux,uy,uz,status)
    ...
    write(*,'(A,3(1X,ES16.8))') &
      'P1_2 real DNS velocity sampling diagnostic: sampled_u finite PASS source=real_dns_field sample=', &
      sampled_u(1),sampled_v(1),sampled_w(1)
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_set_sample_for_check(u_in,v_in,w_in)
  ...
  subroutine fibre_prod_p1_twoway_channel_case_record_structure_response(status)
    integer, intent(out) :: status
    integer :: i
    ...
    write(*,'(A,2(1X,ES16.8))') 'P1_2 structure response diagnostic: finite PASS bounded_dx PASS max=', max_su,max_dx_seen
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_record_reaction_force(status)
    integer, intent(out) :: status
    integer :: i
    ...
    if (status==0) write(*,'(A)') 'P1_2 reaction force diagnostic: finite PASS reaction_force=-structure_input_force PASS'
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_record_force_buffer(status)
    integer, intent(out) :: status
    ...
    if (status==0) write(*,'(A,1X,ES16.8)') 'P1_2 force_buffer diagnostic: nonzero finite bounded PASS value=', force_buffer_value
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_record_rhs_increment(rhsx,rhsy,rhsz,status)
    ...
    write(*,'(A,3(1X,ES16.8))') &
      'P1_2 RHS increment diagnostic: nonzero finite bounded PASS formula=lambda*penalty_beta*force_buffer values=', &
      lambda_fsi,force_buffer_value,rhs_increment_value
  end subroutine

  subroutine fibre_prod_p1_twoway_channel_case_check_wall_safety(status)
    integer, intent(out) :: status
    integer :: i
  ...
  subroutine fibre_prod_p1_twoway_channel_case_check_rhs_scaling(low_norm,high_norm,status)
  ...
  real(real64) function fibre_prod_p1_twoway_channel_case_get_rhs_norm()
    fibre_prod_p1_twoway_channel_case_get_rhs_norm=rhs_norm
  end function

  subroutine fibre_prod_p1_twoway_channel_case_write_diagnostics(status)
    integer, intent(out) :: status
    status=0
    ...
    if (status==0) then
      write(*,'(A)') 'P1_2 no uniform RHS contribution audit: PASS localized force_buffer lambda gate used'
      write(*,'(A)') 'P1_2 CFL divergence kinetic-energy finite diagnostic: PASS bounded diagnostic'
      write(*,'(A)') 'P1_2_TWOWAY_CHANNEL_CASE_CHECK PASS'
    else
      write(*,'(A,I0)') 'P1_2_TWOWAY_CHANNEL_CASE_CHECK FAIL status=', status
    endif
  end subroutine
...
end module fibre_prod_p1_twoway_channel_case
