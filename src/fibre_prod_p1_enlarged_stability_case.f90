module fibre_prod_p1_enlarged_stability_case
  use, intrinsic :: iso_fortran_env, only : real64
  use, intrinsic :: ieee_arithmetic, only : ieee_is_finite
  implicit none
  private
  public :: fibre_prod_p1_enlarged_stability_case_env_enabled
  public :: fibre_prod_p1_enlarged_stability_case_init
  public :: fibre_prod_p1_enlarged_stability_case_init_values
  public :: fibre_prod_p1_enlarged_stability_case_record_sampling
  public :: fibre_prod_p1_enlarged_stability_case_record_structure_response
  public :: fibre_prod_p1_enlarged_stability_case_record_reaction_force
  public :: fibre_prod_p1_enlarged_stability_case_record_force_buffer
  public :: fibre_prod_p1_enlarged_stability_case_record_rhs_increment
  public :: fibre_prod_p1_enlarged_stability_case_check_wall_safety
  public :: fibre_prod_p1_enlarged_stability_case_check_rhs_scaling
  public :: fibre_prod_p1_enlarged_stability_case_write_diagnostics
  public :: fibre_prod_p1_enlarged_stability_case_record_restart_signature
  public :: fibre_prod_p1_enlarged_stability_case_check_restart_continuity
  public :: fibre_prod_p1_enlarged_stability_case_get_rhs_norm
  public :: fibre_prod_p1_enlarged_stability_case_set_sample_for_check

  integer :: fibre_count=1, fibre_nnode=65, sample_count=0, response_count=0
  real(real64) :: lambda_fsi=1.0e-4_real64, penalty_beta=2.0_real64, wall_clearance_min=0.10_real64
  ...
  logical :: restart_initial_recorded=.false., restart_final_recorded=.false.
  real(real64) :: segment1_signature=0.0_real64, restart_initial_signature=0.0_real64, restart_final_signature=0.0_real64
contains
  logical function fibre_prod_p1_enlarged_stability_case_env_enabled()
    fibre_prod_p1_enlarged_stability_case_env_enabled = read_env_logical('FIBRE_PROD_P1_ENLARGED_STABILITY_ENABLE', .false.)
  end function

  subroutine fibre_prod_p1_enlarged_stability_case_init(status)
    integer, intent(out) :: status
    call fibre_prod_p1_enlarged_stability_case_init_values(read_env_int('FIBRE_PROD_P1_FIBRE_COUNT',1), &
      read_env_int('FIBRE_PROD_P1_FIBRE_NNODE',65), read_env_real('FIBRE_PROD_LAMBDA',1.0e-4_real64), &
      read_env_real('FIBRE_PROD_PENALTY_BETA',2.0_real64), &
      read_env_real('FIBRE_PROD_P1_WALL_CLEARANCE_MIN',0.10_real64), &
      read_env_real('FIBRE_PROD_P1_STABILITY_MAX_DX',1.0e-2_real64), &
      read_env_real('FIBRE_PROD_P1_STABILITY_MAX_STRUCTURE_U',1.0e2_real64), &
      read_env_real('FIBRE_PROD_P1_STABILITY_MAX_RHS_INCREMENT',1.0e2_real64), &
      read_env_real('FIBRE_PROD_P1_STABILITY_MAX_FORCE_BUFFER',1.0e6_real64), status)
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_init_values(count_in,nnode_in,lambda_in,beta_in,clearance_in, &
    ...
    restart_initial_recorded=.false.; restart_final_recorded=.false.; segment1_signature=0.0_real64
    restart_initial_signature=0.0_real64; restart_final_signature=0.0_real64
    if (fibre_count /= 1) status=10
    if (status==0 .and. fibre_nnode /= 65) status=11
    ...
    initialized=.true.; call fibre_prod_p1_enlarged_stability_case_check_wall_safety(status)
    if (status /= 0) return
    write(*,'(A,ES12.4)') 'P1_3 fibre initialization diagnostic: count=1 nnode=65 inside_channel=PASS lambda=', lambda_fsi
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_record_sampling(ux,uy,uz,status)
    ...
    write(*,'(A,3(1X,ES16.8))') &
      'P1_3 real DNS velocity sampling diagnostic: sampled_u finite PASS source=real_dns_field sample=', &
      sampled_u(1),sampled_v(1),sampled_w(1)
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_set_sample_for_check(u_in,v_in,w_in)
  ...
  subroutine fibre_prod_p1_enlarged_stability_case_record_structure_response(status)
    integer, intent(out) :: status
    integer :: i
    ...
    write(*,'(A,2(1X,ES16.8))') 'P1_3 structure response diagnostic: finite PASS bounded_dx PASS max=', max_su,max_dx_seen
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_record_reaction_force(status)
    integer, intent(out) :: status
    integer :: i
    ...
    if (status==0) write(*,'(A)') 'P1_3 reaction force diagnostic: finite PASS reaction_force=-structure_input_force PASS'
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_record_force_buffer(status)
    integer, intent(out) :: status
    ...
    if (status==0 .and. abs(force_buffer_value) <= 0.0_real64) status=52
    if (status==0 .and. abs(force_buffer_value) > max_force_buffer) status=53
    force_buffer_ok=(status==0)
    if (status==0) write(*,'(A,1X,ES16.8)') 'P1_3 force_buffer diagnostic: nonzero finite bounded PASS value=', force_buffer_value
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_record_rhs_increment(rhsx,rhsy,rhsz,status)
    ...
    if (status==0 .and. abs(rhs_increment_value) <= 0.0_real64) status=62
    ...
    rhsx(ix,iy,iz)=rhsx(ix,iy,iz)+rhs_increment_value; rhs_norm=abs(rhs_increment_value); rhs_ok=.true.
    write(*,'(A,3(1X,ES16.8))') &
  ...
  subroutine fibre_prod_p1_enlarged_stability_case_check_wall_safety(status)
    integer, intent(out) :: status
    integer :: i
  ...
  subroutine fibre_prod_p1_enlarged_stability_case_check_rhs_scaling(low_norm,high_norm,status)
  ...
  real(real64) function fibre_prod_p1_enlarged_stability_case_get_rhs_norm()
    fibre_prod_p1_enlarged_stability_case_get_rhs_norm=rhs_norm
  end function

  subroutine fibre_prod_p1_enlarged_stability_case_record_restart_signature(phase,status)
    integer, intent(in) :: phase
    integer, intent(out) :: status
    real(real64) :: sig
    status=0
    if (.not.initialized) then; status=85; return; endif
    sig=sum(xf)+sum(sampled_u)+rhs_norm
    select case (phase)
    case (1)
      segment1_signature=sig
    case (2)
      restart_initial_signature=sig; restart_initial_recorded=.true.
    case (3)
      restart_final_signature=sig; restart_final_recorded=.true.
    case default
      status=86
    end select
    if (status==0) write(*,'(A,I0,1X,ES16.8)') 'P1_3 restart signature diagnostic: phase=', phase, sig
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_check_restart_continuity(status)
    integer, intent(out) :: status
    status=0
    if (.not.(restart_initial_recorded .and. restart_final_recorded)) status=87
    if (status==0 .and. .not.ieee_is_finite(restart_initial_signature+restart_final_signature)) status=88
    if (status==0 .and. abs(restart_initial_signature-segment1_signature) > 1.0e-8_real64) status=89
    if (status==0) write(*,'(A)') 'P1_3 restart compatibility diagnostic: signature continuity PASS'
  end subroutine

  subroutine fibre_prod_p1_enlarged_stability_case_write_diagnostics(status)
    integer, intent(out) :: status
    status=0
    ...
    if (status==0) then
      write(*,'(A)') 'P1_3 no uniform RHS contribution audit: PASS localized force_buffer lambda gate used'
      write(*,'(A)') 'P1_3 CFL divergence kinetic-energy finite diagnostic: PASS bounded diagnostic'
      write(*,'(A)') 'P1_3_ENLARGED_STABILITY_CASE_CHECK PASS'
    else
      write(*,'(A,I0)') 'P1_3_ENLARGED_STABILITY_CASE_CHECK FAIL status=', status
    endif
  end subroutine
...
end module fibre_prod_p1_enlarged_stability_case
