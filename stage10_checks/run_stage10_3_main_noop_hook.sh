...
# Do not naked-run xcompact3d here.  Use the already validated Stage 9.9
# deterministic no-fibre gate so that input generation, short-run setup,
# signatures, MPI launcher handling, and output paths remain source-matched.
...
req=$(getv stage10_1_requested_flag "$hook_dat"); noop=$(getv stage10_1_noop_mode_status "$hook_dat")
hi=$(getv stage10_1_hook_init_status "$hook_dat"); hs=$(getv stage10_1_hook_pre_step_status "$hook_dat"); hr=$(getv stage10_1_hook_pre_rhs_status "$hook_dat"); hp=$(getv stage10_1_hook_post_projection_status "$hook_dat"); hps=$(getv stage10_1_hook_post_step_status "$hook_dat"); hf=$(getv stage10_1_hook_finalize_status "$hook_dat")
nf=$(getv stage10_1_no_fibre_state_status "$hook_dat"); nfo=$(getv stage10_1_no_force_status "$hook_dat"); nrhs=$(getv stage10_1_no_rhs_injection_status "$hook_dat"); nibm=$(getv stage10_1_no_ibm_call_status "$hook_dat"); nst=$(getv stage10_1_no_structure_advance_status "$hook_dat")
...
[ "$s10_0" = "1" ] || main_status=0
[ "$s10_1" = "1" ] || main_status=0
[ "$s10_2" = "1" ] || main_status=0
[ -s "$s99" ] || main_status=0
